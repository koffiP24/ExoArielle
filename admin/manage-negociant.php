                ...
                <?php if ($negociants && mysqli_num_rows($negociants) > 0): ?>
                    <?php while ($row = mysqli_fetch_assoc($negociants)): ?>
                        <tr>
                            ...
                            <td><form method="POST"><input type="hidden" name="action" value="delete"><input type="hidden" name="id" value="<?php echo e($row["idNegociant"]); ?>"><button class="btn-delete" onclick="return confirm('Confirmer la suppression ?')"><i class="fa-solid fa-trash"></i></button></form></td>
                        </tr>
                    <?php endwhile; ?>
                <?php else: ?>
